...
import (
	...
	"github.com/fullerhkz/qui-transmission/internal/api"
	...
	"github.com/fullerhkz/qui-transmission/internal/metrics"
	...
	"github.com/fullerhkz/qui-transmission/internal/services/orphanscan"
	"github.com/fullerhkz/qui-transmission/internal/services/reannounce"
	...
	"github.com/fullerhkz/qui-transmission/pkg/sqlite3store"
)
...
func main() {
	...
	var rootCmd = &cobra.Command{
		Use:   "qui-transmission",
		Short: "A self-hosted Transmission WebUI alternative",
		Long: `qui-Transmission - A modern, self-hosted web interface for managing
multiple Transmission instances with support for large torrent libraries.`,
	}
...
func RunServeCommand() *cobra.Command {
	...
	command.Flags().StringVar(&configDir, "config-dir", "", "config directory path (default is OS-specific: ~/.config/qui-transmission/ or %APPDATA%\\qui-transmission\\). For backward compatibility, can also be a direct path to a .toml file")
...
func RunVersionCommand(version string) *cobra.Command {
	var command = &cobra.Command{
		Use:   "version",
		Short: "Print the version number of qui-Transmission",
...
- Directory: qui-transmission generate-config --config-dir /path/to/config/
- File: qui-transmission generate-config --config-dir /path/to/myconfig.toml`,
...
func RunUpdateCommand() *cobra.Command {
	var command = &cobra.Command{
		Use:                   "update",
		Short:                 "Update qui-Transmission",
		Long:                  `Update qui-Transmission to the latest version.`,
		DisableFlagsInUseLine: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			updater := update.NewUpdater(update.Config{
				Repository: "fullerhkz/qui-transmission",
...
func (app *Application) runServer() {
	...
	log.Info().Str("version", buildinfo.Version).Msg("Starting qui-Transmission")
	...
	case cfg.Config.IsAuthDisabled():
		...
		log.Warn().Strs("authDisabledAllowedCIDRs", cfg.Config.AuthDisabledAllowedCIDRs).Msg("Authentication is disabled via QUI__AUTH_DISABLED. Access is restricted to authDisabledAllowedCIDRs. Make sure qui-Transmission is behind a reverse proxy with its own authentication.")
	...
	// Initialize Transmission client pool
