...
func NewClientWithTimeout(instanceID int, instanceHost, username, password, apiKey string, basicUsername, basicPassword *string, tlsSkipVerify bool, timeout time.Duration) (*Client, error) {
	...
	if err := client.RefreshCapabilities(ctx); err != nil {
		log.Warn().
			...
			Msg("Failed to refresh Transmission capabilities during client creation")
	...
	log.Debug().
		...
		Msg("Transmission client created successfully")
...
func (c *Client) RefreshCapabilities(ctx context.Context) error {
	...
	if previousVersion != "" && previousVersion != version {
		log.Info().
			...
			Msg("Transmission RPC version changed, refreshed capabilities")
	}
...
func (c *Client) applyCapabilitiesLocked(version string) {
	...
	// qui-Transmission is backed by Transmission RPC, not qBittorrent WebAPI.
	// Keep the existing capability contract for the frontend, but expose only
	// features that have a direct Transmission mapping in the compatibility
	// client.
	c.supportsSetTags = true
	c.supportsSetComment = true
	c.supportsTorrentCreation = false
	c.supportsTorrentExport = true
