...
import (
	...
	"github.com/fullerhkz/qui-transmission/pkg/torrentname"
)
...
func (h *TorrentsHandler) CheckDuplicates(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		log.Error().Err(err).Int("instanceID", instanceID).Msg("Failed to get Transmission sync manager")
	...
	if len(torrentFiles) > 0 {
		...
		if indexerID > 0 {
			...
			for _, url := range urls {
				...
				// Magnet links can be added directly to Transmission.
				if strings.HasPrefix(strings.ToLower(url), "magnet:") {
					...
					if err != nil {
						...
						log.Error().
							...
							Msg("Transmission reported failed magnet link add")
						failedURLs = append(failedURLs, failedURL{URL: url, Error: err.Error()})
						failedCount++
						lastError = err
					} else {
					...
					if errors.As(err, &magnetErr) && magnetErr.MagnetURL != "" {
						...
						if err != nil {
							...
							log.Error().
								...
								Int64("failureCount", resp.FailureCount).
								Int64("pendingCount", resp.PendingCount).
								Msg("Transmission reported failed magnet link from indexer redirect")
							failedURLs = append(failedURLs, failedURL{URL: magnetURL, Error: err.Error()})
							failedCount++
							lastError = err
						} else {
			...
			// (works for local Transmission instances or magnet links)
			var skippedEmpty int
			for _, url := range urls {
				url = strings.TrimSpace(url)
				if url == "" {
				...
				if err := torrentURLAddFailureError(resp); err != nil {
					log.Error().
						...
						Int64("failureCount", resp.FailureCount).
						Int64("pendingCount", resp.PendingCount).
						Msg("Transmission reported failed URL add")
					failedURLs = append(failedURLs, failedURL{URL: url, Error: err.Error()})
					failedCount++
					lastError = err
					continue
...
func torrentURLAddFailureError(resp *qbt.TorrentAddResponse) error {
	...
	return errors.New("Transmission rejected torrent URL")
}
...
func (h *TorrentsHandler) CreateTorrent(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.Is(err, qbt.ErrUnsupportedVersion) {
			RespondError(w, http.StatusBadRequest, "Torrent creation is not supported by Transmission RPC in qui-Transmission.")
			return
		}
