...
const (
	osWindows                           = "windows"
	errMsgWindowsPathSourceNotSupported = "Path-based free space source is not supported on Windows. Use the default Transmission free space instead."
)
...
func validateFreeSpaceSource(source *models.FreeSpaceSource, instance *models.Instance, usesFreeSpace bool) (status int, msg string, err error) {
	...
	default:
		return http.StatusBadRequest, "Invalid free space source type. Use 'qbittorrent' (the default Transmission free space source) or 'path'.", errors.New("invalid source type")
	}
}
