...
func (s *Service) executeBackup(ctx context.Context, j job) (*backupResult, error) {
	...
	if version, err := s.reader.GetInstanceWebAPIVersion(ctx, j.instanceID); err != nil {
		log.Debug().Err(err).Int("instanceID", j.instanceID).Msg("Unable to determine Transmission RPC version for tracker patching")
	} else {
		...
		if data == nil {
			if shouldSkipLiveExportForBackup(torrent, cachedTorrent != nil, cacheErr) {
				log.Warn().
					...
					Msg("Skipping torrent export; live Transmission export disabled for hybrid torrents")
...
// response time when Transmission is under load. This prevents overwhelming
// Transmission during large backup operations.
			...
			if torrentPaths != nil && item.ArchivePath != "" {
				if tempPath, ok := torrentPaths[item.ArchivePath]; ok {
					if err := s.copyTorrentFromTemp(tempPath, absPath); err == nil {
						...
						log.Warn().Err(err).Str("hash", item.Hash).Msg("Failed to copy from temp, will try Transmission")
					}
				}
			}
