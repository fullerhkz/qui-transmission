...
func (c *AppConfig) defaults() {
	...
	c.viper.SetDefault("databaseName", "qui_transmission")
...
func (c *AppConfig) applyDynamicChanges(previousAuthSettings authReloadSettings) {
	...
	case c.Config.IsAuthDisabled():
		log.Warn().Strs("authDisabledAllowedCIDRs", c.Config.AuthDisabledAllowedCIDRs).Msg("Authentication is disabled via QUI__AUTH_DISABLED. Access is restricted to authDisabledAllowedCIDRs. Make sure qui-Transmission is behind a reverse proxy with its own authentication.")
...
# Base URL
# Set custom baseUrl eg /qui-transmission/ to serve in subdirectory.
...
# If changed, you'll need to re-enter passwords for all existing Transmission instances in the UI.
...
# Database file (qui-transmission.db) will be created inside this directory
#dataDir = "/var/db/qui-transmission"
...
#databaseDsn = "postgres://user:password@localhost:5432/qui_transmission?sslmode=disable"
#databaseHost = "localhost"
#databasePort = 5432
#databaseUser = ""
#databasePassword = ""
#databaseName = "qui_transmission"
...
# Disable to prevent qui-Transmission from requesting tracker favicons from remote trackers.
# Default: true
...
# Restrict which executables can be started from qui-Transmission.
...
func GetDefaultConfigDir() string {
	...
	if xdgConfig := os.Getenv("XDG_CONFIG_HOME"); xdgConfig != "" {
		...
		// Otherwise append qui-transmission subdirectory
		return filepath.Join(xdgConfig, "qui-transmission")
	}

	switch runtime.GOOS {
	case "windows":
		// Use %APPDATA%\qui-transmission on Windows
		if appData := os.Getenv("APPDATA"); appData != "" {
			return filepath.Join(appData, "qui-transmission")
		}
		// Fallback to home directory
		if home, err := os.UserHomeDir(); err == nil {
			return filepath.Join(home, "AppData", "Roaming", "qui-transmission")
		}
		return filepath.Join(".", "qui-transmission")
	default:
		// Use ~/.config/qui-transmission for Unix-like systems
		if home, err := os.UserHomeDir(); err == nil {
			return filepath.Join(home, ".config", "qui-transmission")
		}
		return filepath.Join(".", ".config", "qui-transmission")
	}
}
...
func (c *AppConfig) GetDatabasePath() string {
	return filepath.Join(c.dataDir, "qui-transmission.db")
}
