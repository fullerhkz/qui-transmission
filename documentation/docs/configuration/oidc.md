...
If your provider advertises PKCE (`S256`) support, qui-Transmission uses it automatically for the authorization flow. No extra qui-Transmission setting is required.
...
qui-Transmission does not currently emit a dedicated "PKCE enabled" log line, so the authorize URL is the easiest check.
...
| `QUI__OIDC_CLIENT_ID` | Client ID registered for qui-Transmission |
...
```
https://host/qui-Transmission/api/auth/oidc/callback
```
...
QUI__OIDC_CLIENT_ID=qui-Transmission \
QUI__OIDC_CLIENT_SECRET=super-secret-value \
QUI__OIDC_REDIRECT_URL=https://qui-Transmission.example.com/api/auth/oidc/callback \
QUI__OIDC_DISABLE_BUILT_IN_LOGIN=true
```

You can set the same options in `config.toml` using the `oidc*` keys generated by `qui-Transmission generate-config`.
