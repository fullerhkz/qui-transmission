...
| Download Path | Session download path from Transmission |
...
These fields use qui-Transmission's current system time when the rule is evaluated. They are useful for time-window automations such as "only run at night" or "apply different actions on weekends."
...
| Max Ratio   | Transmission max ratio value  |
...
Note: if you have **Settings â†’ Tracker Customizations** configured, the **Tracker** condition can match the display name in addition to the raw URL/domain.
...
| Field                            | Description                                                                                                    |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Hardlink Scope                   | `none`, `torrents_only`, or `outside_Transmission` (requires local filesystem access)                         |
| Hardlink Scope (Cross-Instance)  | `none`, `torrents_only`, or `outside_Transmission` considering ALL instances (requires local filesystem access) |
...
- If a grouped condition row has no `groupId`, qui-Transmission uses `conditions.grouping.defaultGroupId`.
...
Some actions accept a `groupId`. When set, qui-Transmission expands the action to all torrents in that group.
...
- If any member fails (or the group cannot be resolved), qui-Transmission skips the entire grouped action.
...
| Unlimited | 0     | Remove speed limit (Transmission treats 0 as unlimited) |
...
| ---------- | ----- | -------------------------------------------------- |
| No change  | -     | Don't modify this field                            |
| Use global | -2    | Follow Transmission's global share settings         |
...
**Limits matching mode** (match **any** limit vs **all** limits) is a separate Web API capability and requires **2.16.0** or newer. On slightly older 5.2 builds that only expose **2.15.1**, qui-Transmission still shows the action above but hides this control until you upgrade Transmission. Values use **string enum names** for `setShareLimits`:
...
These options are hidden when the instance does not report the required Web API version (see [Transmission Version Compatibility](../advanced/compatibility.md)). Ratio and seeding time limits above still apply on older instances; only the extra controls are gated.

These fields appear in both the torrent share limit dialog and the automation workflow editor when the connected Transmission instance supports them. On older instances, the fields are hidden and only the classic ratio/seeding time limits are sent.
...
- Triggers Transmission recheck for matched torrents.
...
Remove torrents from Transmission. **Must be standalone** - cannot combine with other actions.
...
- **Safe expansion**: If qui-Transmission can't safely confirm another torrent uses the same files, it won't be included in the deletion.
...
- **Safe scope**: Only includes hardlinks that are fully contained within Transmission's torrent set. Never follows hardlinks to files outside Transmission (e.g., your media library).
...
This is useful when you have hardlinked copies of content across different locations in Transmission and want to clean up all copies together.
...
- **Managed / Replace in Client** - `Managed` (default) applies per-torrent add/remove diffs only. `Replace in client` deletes managed tags from Transmission first, then reapplies to current matches.
...
- By category: `/data/{{.Category}}` â†’ e.g. `/data/movies`
...
Enable or disable Transmission's Automatic Torrent Management (AutoTMM) on matching torrents.
...
When AutoTMM is enabled, Transmission automatically moves torrents to the save path configured for their category. Disabling it allows manual control of save paths.

If multiple rules match the same torrent with Auto Management actions, the **last matching rule** (by sort order) wins.

### External Program

Run a pre-configured external program when torrents match the automation rule. Uses the same programs configured in **Settings â†’ External Programs**.
...
:::note
The program must be enabled in Settings â†’ External Programs to appear in the automation dropdown.
:::
...
:::warning
The program's executable path must be present in the application's allowlist. Programs that are disabled or have forbidden paths will not runâ€”attempts are rejected and logged in the activity log with the rule name and torrent details.
:::
...
Export a torrent's `.torrent` file from the current instance and add it to a different Transmission instance. This is useful for migrating torrents between instances â€” for example, moving from a seedbox to a local instance for long-term seeding.
...
| Field              | Description                                                              |
| ------------------ | ------------------------------------------------------------------------ |
| **Target instance** | Destination Transmission instance (cannot be the same as source)         |
...
- **Cannot combine with Delete** â€” the API rejects rules that have both export and delete enabled
- Duplicate detection: before exporting, qui-Transmission checks if the torrent already exists on the target instance and skips it if found
- After adding to the target, qui-Transmission verifies the torrent appeared and is healthy. If verification fails, the torrent is automatically cleaned up from the target so it can be retried on the next run
...
- Dry-run is supported â€” shows what would be exported without actually transferring
...
| `.Name`                | Torrent display name                                                                     |
| `.Hash`                | Info hash                                                                                |
| `.Category`            | Transmission category (on source instance)                                                |
| `.IsolationFolderName` | Filesystem-safe folder name (hash or sanitized name)                                     |
| `.Tracker`             | Tracker display name (when available from instance config), otherwise the tracker domain |
...
If no save path is set but a category is configured, Transmission's Automatic Torrent Management is enabled so the target uses the category's configured path.
...
The `HARDLINK_SCOPE` field lets automations distinguish between torrents whose files are hardlinked into an external library (Sonarr, Radarr, etc.) and torrents that exist only within Transmission. This is the foundation for safe "Remove Upgraded Torrents" automations.

#### How scope is determined

When an automation references `HARDLINK_SCOPE`, qui-Transmission builds a hardlink index by calling `Lstat()` on every file of every torrent in Transmission. For each file it extracts:

- The **inode** and **device ID** â€” uniquely identifying the file on disk.
- The **nlink count** â€” the total number of hardlinks to that inode, as reported by the filesystem.

It then counts how many unique file paths across the entire Transmission torrent set point to each inode. The scope for each torrent is determined by comparing these two numbers:

| Scope                 | Condition                                                                    | Meaning                                                                                           |
| --------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `none`                | No file has `nlink > 1`                                                      | No hardlinks detected.                                                                            |
| `torrents_only`       | At least one file has `nlink > 1`, and no file has `nlink > uniquePathCount` | Hardlinks exist, but only between torrents in Transmission. No external library links.             |
| `outside_Transmission` | Any file has `nlink > uniquePathCount`                                       | Something outside Transmission has hardlinked the file â€” typically a Sonarr/Radarr library import. |
...
If qui-Transmission cannot `Lstat()` **any** file in a torrent â€” due to wrong paths, missing permissions, or inaccessible storage â€” that torrent receives no scope entry. All `HARDLINK_SCOPE` conditions evaluate to `false` for that torrent, regardless of the operator or value. This is a safety measure to prevent unintended deletions of torrents qui-Transmission cannot fully inspect.
...
1. **Paths must match exactly.** qui-Transmission must be able to read files at the same paths Transmission reports. If Transmission says a torrent's save path is `/data/torrents/radarr/`, qui-Transmission must be able to access `/data/torrents/radarr/` inside its container.

2. **Same underlying storage.** Both containers must share the same host mount so that inode numbers are consistent. If qui-Transmission and Transmission access the same files through different host mounts or different bind-mount configurations, inode numbers may not match.
...
services:
  qui-Transmission:
    volumes:
      - /home/user/docker/qui-Transmission:/config
      - /mnt/media/data:/data # single mount covering both torrents and library
```

Avoid mounting both `/mnt/media/data/torrents:/data/torrents` **and** `/mnt/media/data:/data` â€” the overlapping mounts can cause inconsistent inode visibility. Use a single mount at the common parent.
...
On affected filesystems, every torrent appears to have scope `none` because nlink is always 1. There is no workaround within qui-Transmission â€” this is a kernel/filesystem limitation. If you suspect this issue, run `stat` on a file you know is hardlinked and check the "Links" count.
...
:::tip
Use `HARDLINK_SCOPE` with `NOT_EQUAL` to `outside_Transmission` rather than `EQUAL` to `none`. This way torrents with scope `torrents_only` (cross-seeded but not in a library) are also eligible for cleanup, while any torrent still linked into your media library is protected.
:::
...
The `HARDLINK_SCOPE_CROSS` field extends hardlink detection across **all** configured Transmission instances. While `HARDLINK_SCOPE` only considers torrents within a single instance, `HARDLINK_SCOPE_CROSS` accounts for hardlinks to files managed by any instance with local filesystem access enabled.

This is essential for multi-instance setups where cross-seeds are hardlinked across instances. Without cross-instance awareness, those hardlinks appear as `outside_Transmission` even though they point to files managed by another Transmission instance.
...
| `torrents_only`       | All hardlinks are accounted for across all Transmission instances.        |
| `outside_Transmission` | Hardlinks exist to files outside all Transmission instances.              |
...
| --- | --- |
| `HARDLINK_SCOPE = outside_Transmission` AND `HARDLINK_SCOPE_CROSS = torrents_only` | Hardlinks point to other Transmission instances only (cross-seeds). No media library copy. |
| `HARDLINK_SCOPE = outside_Transmission` AND `HARDLINK_SCOPE_CROSS = outside_Transmission` | Hardlinks point outside all instances â€” typically a media library import. |
...
1. **Local Filesystem Access** enabled on **all** instances whose files you want considered. Instances without it are skipped â€” their files won't be scanned, and unresolved hardlinks will conservatively show as `outside_Transmission`.
2. **Same filesystem** across all instances. Hardlinks cannot cross filesystem boundaries.
3. **Matching paths in Docker** â€” same volume mount requirements as `HARDLINK_SCOPE`, applied to every instance.
...
This works because `HARDLINK_SCOPE_CROSS != outside_Transmission` matches both `none` (no hardlinks) and `torrents_only` (hardlinks only between Transmission instances). Torrents with a media library copy (`outside_Transmission`) are excluded from the tag.
...
:::note
After removing files, qui-Transmission waits ~5 minutes before running Free Space deletes again to allow Transmission to refresh its disk free space reading. The UI prevents selecting 1 minute intervals for Free Space delete rules.
:::

#### Free Space Source

By default, Free Space uses Transmission's reported free space (based on its default download location). If you have multiple disks or want to manage a specific mount point, select "Path on server" and enter the path to that disk.

| Source                | Description                                      |
| --------------------- | ------------------------------------------------ |
| Default (Transmission) | Uses Transmission's reported free space           |
...
:::note
On Windows, Path on server is not supported and Free Space always uses Transmission's reported free space. The UI disables the option and switches legacy workflows back to the default when opened.
:::

### Batching

Torrents are grouped by action value and sent to Transmission in batches of up to 50 hashes per API call.
