...
sidebar_position: 1
title: Backups
description: Schedule and restore Transmission instance backups.
---

# Backups & Restore

qui-Transmission can take scheduled or ad-hoc snapshots of a Transmission instance. Each snapshot includes the torrent archive, tags, categories (with save paths), and cached `.torrent` blobs so that you can recreate the original state later.
...
Safest option. Creates any categories, tags, or torrents that are missing from the live instance but never modifies or removes existing data. Use this when you just want to seed new items into an active Transmission without touching what is already there.
...
Downloaded backups can be imported into any qui-Transmission instance. Useful for migrating to a new server or recovering after data loss. Click **Import** on the Backups page and select the backup file. All export formats are supported.
