...
qui-Transmission includes intelligent cross-seeding capabilities that help you automatically find and add matching torrents across different trackers. This allows you to seed the same content on multiple trackers.

## How It Works

When you cross-seed a torrent, qui-Transmission:
...
qui-Transmission supports three modes for handling files:
...
- **Reflink mode** (optional): Creates copy-on-write clones (reflinks) of the matched files. Allows safe cross-seeding of torrents with extra/missing files because Transmission can write/repair the clones without affecting originals.
...
You need Prowlarr or Jackett to provide Torznab indexer feeds. Add your indexers in **Settings â†’ Indexers** using the "1-click sync" feature to import from Prowlarr/Jackett automatically.

Optional: qui-Transmission can also query OPS/RED directly via the trackers' Gazelle JSON APIs. This complements Torznab, can handle OPS/RED searches even when no Torznab backend is available, and excludes OPS/RED Torznab indexers for per-torrent searches only when **both** Gazelle keys are configured. See [OPS/RED (Gazelle)](./gazelle-ops-red.md).

**Optional but recommended:** Configure Sonarr/Radarr instances in **Settings â†’ Integrations** to enable external ID lookups (IMDb, TMDb, TVDb, TVMaze). When configured, qui-Transmission queries your *arr instances to resolve IDs for cross-seed searches, improving match accuracy on indexers that support ID-based queries.
- This is especially helpful for content that is "AKA" type, and can have differing names depending on locale.

## Discovery Methods

qui-Transmission offers several ways to find cross-seed opportunities:
...
- **Target instances** - Which Transmission instances receive cross-seeds
...
- **Source instance** - The Transmission instance to scan
...
- **Cooldown** - Skip torrents searched within this window (minimum 12 hours). qui-Transmission records this only after an actual remote Gazelle or Torznab request, so local preflight failures and local Gazelle skips do not suppress future searches.
...
- **Search delay** - Wait 0-600 seconds after completion before searching. Default: 0. Use this when post-completion file moves or sister-torrent injection tools need a short head start before qui-Transmission searches trackers.

If a torrent is still **checking** or **moving**, qui-Transmission waits and runs the completion search afterward instead of searching immediately against an unstable path/state.
...
Assemble season-pack torrents from individual episodes you already seed. When autobrr announces a season pack, qui-Transmission checks your Transmission instances for matching episodes, links whatever is already local, and lets Transmission download the remainder after recheck when coverage passes the configured threshold (default 75%). Sonarr, TVDB, and TVMaze improve the threshold decision when available. Requires local filesystem access and hardlink/reflink mode. See [Season Packs](./season-packs.md) for setup.

## Blocklist

Use the per-instance blocklist to prevent specific infohashes from being injected again.

- **Manage**: Cross-Seed page â†’ Blocklist tab
