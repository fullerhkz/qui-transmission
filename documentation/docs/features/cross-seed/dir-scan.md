...
Directory Scanner (Dir Scan) scans local folders to find cross-seed opportunities for content already on disk. Unlike Library Scan (which queries Transmission's torrent list), Dir Scan works directly with files on the filesystem.

Configure it in **Cross-Seed > Dir Scan**.

## Requirements

- At least one Transmission instance must have **Local filesystem access** enabled in Instance Settings.
- qui-Transmission must be able to read the files directly (same host or shared mounts as the target Transmission instance).
...
```plaintext
/data/media/movies/
â”œâ”€â”€ Movie.2024.1080p.BluRay/   <- searchee 1
â”‚   â”œâ”€â”€ movie.mkv
â”‚   â””â”€â”€ movie.nfo
â”œâ”€â”€ Another.Movie.2023.2160p/  <- searchee 2
â”‚   â””â”€â”€ movie.mkv
â””â”€â”€ standalone.mkv             <- searchee 3
```
...
When qui-Transmission and Transmission run in separate containers or see different mount points, you need path mapping.

### "Local filesystem access" explained

Enabling **Local filesystem access** on a Transmission instance tells qui-Transmission:
1. qui-Transmission can read files directly from the filesystem (same paths or mapped paths).
2. qui-Transmission should use file-based matching (inode checks, size verification) rather than relying solely on Transmission's API.

This requires qui-Transmission to have read access to the actual files, either on the same host or via shared network/volume mounts.
...
When both containers see `/data/media/movies`, no path mapping is needed. Leave **Transmission Path Prefix** empty.

### Path mapping example (different mount points)

Your setup:
- qui-Transmission container mounts: `-v /mnt/storage:/data`
- Transmission container mounts: `-v /mnt/storage:/downloads`

qui-Transmission sees files at `/data/media/movies/Movie.2024/movie.mkv`
Transmission sees the same file at `/downloads/media/movies/Movie.2024/movie.mkv`

Configure Dir Scan:
- **Directory Path**: `/data/media/movies`
- **Transmission Path Prefix**: `/downloads/media/movies`

When qui-Transmission finds a match, it tells Transmission to add the torrent pointing at `/downloads/media/movies/Movie.2024/` instead of `/data/media/movies/Movie.2024/`.

## How It Works

For each configured scan directory, qui-Transmission:
...
7. If a match is found, adds the torrent to the target Transmission instance.

:::note Categories + AutoTMM
Dir Scan adds torrents using an explicit `savepath` to point Transmission at the existing files on disk. That forces **AutoTMM off** for Dir Scan injections.

Dir Scan categories come only from **Dir Scan â†’ Default Category** and per-directory **Category override**. Cross-Seed â†’ Rules category modes (affix / indexer / custom) do not apply to Dir Scan.

If you later enable AutoTMM on an injected torrent, Transmission may relocate files based on its default save path + category rules.
:::
...
Dir Scan maintains a FileID index (inode + device on Unix) to track files already present in Transmission. It skips:
- Files that are already part of a seeding torrent
- Torrents whose infohash already exists in Transmission

This avoids redundant searches and duplicate additions.

If a torrent is removed from Transmission (for example, by an automation rule that removes torrents with missing files), its files are no longer tracked in the index. The next scan of whichever directory contains those files will treat them as new searchees and search indexers for them again.

### Recheck Behavior

- **Full matches**: Torrent is added with "skip hash check" enabled. Seeding starts immediately.
- **Partial matches** (when enabled): Torrent is added without skipping hash check. Transmission verifies existing data and downloads missing files.
...
- Season-pack searches are kept only when all episode files in that season work item are within the cutoff; otherwise qui-Transmission falls back to fresh episode-level work only.
- Cutoff is computed as `now - N days` (for example, `7` means â€œolder than 7 daysâ€).
- The timestamp used is filesystem **modified time (mtime)** from video/audio files only, not subtitles, extras, release date, or Transmission add time.
...
| Setting | Description |
|---------|-------------|
| Directory Path | The path qui-Transmission scans (immediate children become searchees). |
| Transmission Path Prefix | Path mapping for container setups. See [Docker and Path Mapping](#docker-and-path-mapping). |
| Target Transmission Instance | Where matched torrents are added. Must have Local filesystem access enabled. |
...
Dir Scan persists per-file progress and skips unchanged searchees whose files are already in a final state (matched/no match/already seeding/in Transmission).

This is **not** an exact checkpoint resume. When you start a new run after canceling or restarting qui-Transmission, Dir Scan:

- rechecks the directory from the top
- keeps finished files skipped if they are unchanged
- retries unfinished or errored files

From a user perspective, this behaves like **restart with preserved progress**, not â€œcontinue from the exact file where it stopped.â€
...
You can trigger a scan automatically when Sonarr, Radarr, Lidarr, or Readarr imports content. The webhook endpoint natively understands *arr webhook payloads â€” no custom scripts needed.

```http
POST /api/dir-scan/webhook/scan?apikey=YOUR_API_KEY
```

qui-Transmission extracts the path from the *arr payload (`series.path`, `movie.folderPath`, `artist.path`, or `author.path`), matches it against the Dir Scan **Directory Path** values configured in qui-Transmission, and uses the provided path itself as the scan root. It does not use Transmission path prefixes for this lookup. On success, the response includes `runId`, `directoryId`, `directoryPath`, and `scanRoot`.
...
1. Go to **Settings â†’ Connect â†’ Add â†’ Webhook**.
2. Set **Name** to something like `qui-Transmission Dir Scan`.
...
4. Set **Webhook URL** to:
   ```text
   http://your-qui-Transmission-host:7476/api/dir-scan/webhook/scan?apikey=YOUR_API_KEY
   ```
...
qui-Transmission uses longest-prefix matching against the configured Dir Scan **Directory Path** values to choose which directory settings apply. The actual scan root is the path from the webhook payload. For example, if you have directories configured for `/data/media/movies` and `/data/media/tv`, and Sonarr sends `series.path: "/data/media/tv/Show Name"`, qui-Transmission matches `/data/media/tv` and scans `/data/media/tv/Show Name`.

In split-mount setups, the *arr app must send the same library path that qui-Transmission sees on disk. If Sonarr/Radarr uses a different mount path than qui-Transmission, the webhook will not find a matching directory.
...
| `202` | Scan accepted. If the directory is idle, qui-Transmission starts the run immediately. If a webhook scan is already running for that directory, qui-Transmission keeps one follow-up queued run and merges later webhook paths into it. Example: `{"runId": 42, "directoryId": 3, "directoryPath": "/data/media/tv", "scanRoot": "/data/media/tv/Show Name"}` |
...
| `500` | Internal server error â€” scan could not be started due to an internal failure |

If a second webhook arrives while the same directory is already scanning, qui-Transmission returns `202` again. It does not reject the request or require client-side retries. Instead, it updates one queued follow-up run for that directory and expands the queued `scanRoot` to the nearest common ancestor when needed.
...
- Add exact client names as shown in Sonarr or Radarr, such as `SABnzbd`, `NZBGet`, or `Transmission`.
- Matching is case-insensitive and ignores leading/trailing whitespace.
- If the webhook is otherwise valid but the client is missing or not allowed, qui-Transmission returns `200` and skips starting a scan.
...
3. **Injecting** - Adding matched torrents to Transmission
...
If the target Transmission instance has hardlink or reflink mode enabled, Dir Scan uses the same behavior as other cross-seed methods:

- Builds a link tree matching the incoming torrent's layout.
- Adds the torrent pointing at that tree (`contentLayout=Original`). Full matches use `skip_checking=true`; partial matches allow Transmission to verify existing data and download missing files safely into the link tree.
...
Filesystem fallback adds the torrent against the matched source files instead of the link-tree directory, so qui-Transmission requires a full 100% recheck before auto-resume. If **Skip recheck** is enabled, the fallback candidate is skipped.

For partial or otherwise non-perfect fallback matches, qui-Transmission runs piece-boundary protection before adding the torrent. This fallback check is always enforced, even when **Skip piece boundary safety check** is enabled for regular reuse mode.
...
:::warning
With **Allow partial matches** enabled, Transmission may download missing files (extras like `.nfo`, subtitles) directly into your *arr-managed library folder. This can create unexpected files alongside your media.
:::
...
**Permissions errors:**
- qui-Transmission must have read access to the scan path.
- Check container volume mounts if running in Docker.

**Wrong path mapping:**
- Verify Transmission Path Prefix matches how Transmission sees the same files.
- Test by checking a torrent's save path in Transmission's UI.
