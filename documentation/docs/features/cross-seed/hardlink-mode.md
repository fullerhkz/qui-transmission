...
Hardlink mode is an opt-in cross-seeding strategy that creates a hardlinked copy of the matched files laid out exactly as the incoming torrent expects, then adds the torrent pointing at that hardlink tree. This can make cross-seed alignment simpler and faster, because Transmission can start seeding immediately without file rename alignment.
...
- You want to avoid Transmission rename-alignment and hash rechecks for layout differences.

## Requirements

- Requires **Local filesystem access** on the target Transmission instance.
...
- qui-Transmission must be able to read the instance's content paths and write to the hardlink base directory.

:::tip Multi-filesystem setups
If your downloads span multiple filesystems (e.g., `/mnt/disk1`, `/mnt/disk2`), you can specify **multiple base directories** separated by commas. qui-Transmission will automatically select the first directory that's on the same filesystem as the source files.
...
- Hardlink mode is a **per-instance setting** (not per request). Each Transmission instance can have its own hardlink configuration.
...
- When fallback handles a partial or otherwise non-perfect match, qui-Transmission runs a piece-boundary safety check before adding the torrent to Transmission. This fallback check is always enforced, even if **Skip piece boundary safety check** is enabled for regular reuse mode.
...
Configure in Cross-Seed â†’ Hardlink Mode â†’ (select instance):

- **Hardlink base directory**: Path(s) on the qui-Transmission host where hardlink trees are created. For multi-filesystem setups, specify multiple paths separated by commas (e.g., `/mnt/disk1/cross-seed, /mnt/disk2/cross-seed`).
- **Directory preset**:
...
For `by-tracker` and `by-instance` presets, qui-Transmission determines whether an isolation folder is needed based on the torrent's file structure:

- **Torrents with a root folder** (e.g., `Movie/video.mkv`, `Movie/subs.srt`) â†’ files already have a common top-level directory, no isolation folder needed
- **Rootless torrents** (e.g., `video.mkv`, `subs.srt` at top level) â†’ isolation folder added to prevent file conflicts
...
1. Enable "Local filesystem access" on the Transmission instance in Instance Settings.
2. In Cross-Seed â†’ Hardlink Mode, expand the instance you want to configure.
...
When hardlink/reflink mode creates a complete link tree with no extra files to download, qui-Transmission adds the torrent with hash checking skipped and does not trigger an automatic recheck. If Transmission instead reports `missing files`, see [Hardlink/reflink cross-seed shows "missing files"](./troubleshooting.md#hardlinkreflink-cross-seed-shows-missing-files).

When the incoming torrent has extra files that are not present in the matched torrent, qui-Transmission adds the torrent paused, triggers a recheck, and resumes it only after Transmission reports progress at or above the configured threshold.

If hardlink/reflink mode falls back to regular mode for a partial or non-perfect match, the fallback add is stricter: qui-Transmission first checks piece boundaries, then adds the torrent paused only when the check passes. Safe fallback adds require a full 100% recheck before auto-resume.
...
- Windows support: folder names are sanitized to remove characters Windows forbids. Torrent file paths themselves still need to be valid for your Transmission setup.
- Hardlink mode supports extra files when piece-boundary safe. If the incoming torrent contains extra files not present in the matched torrent (e.g., `.nfo`/`.srt` sidecars), hardlink mode will link the content files and trigger a recheck so Transmission downloads the extras. If extras share pieces with content (unsafe), the cross-seed is skipped.
...
Reflink mode creates copy-on-write clones of the matched files. Unlike hardlinks, reflinks allow Transmission to safely modify the cloned files (download missing pieces, repair corrupted data) without affecting the original seeded files.

**Key advantage:** Reflink mode **bypasses piece-boundary safety checks**. This means you can cross-seed torrents with extra/missing files even when those files share pieces with existing contentâ€”the clones can be safely modified.
...
- **Local filesystem access** must be enabled on the target Transmission instance.
...
:::note
Windows reflink mode uses ReFS block cloning (requiring a ReFS filesystem). NTFS is not supported. If the matched source path is a symlink, qui-Transmission resolves it before cloning, and the resolved source plus the reflink base directory still need to be on the same ReFS volume. If reflink creation fails, fallback still depends on the existing "Fallback to regular mode" setting.
:::
...
- When Transmission writes to a clone (downloads extras, repairs pieces), only modified blocks are copied
...
:::note
Hardlink and reflink modes are mutually exclusiveâ€”only one can be enabled per instance.
:::
